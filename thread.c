 ...
 */
item *item_alloc(char *key, size_t nkey, int flags, rel_time_t exptime, int nbytes, uint32_t priority) {
    item *it;
    pthread_mutex_lock(&cache_lock);
    it = do_item_alloc(key, nkey, flags, exptime, nbytes, priority);
 ...
 * 获取优先级最高的数据
 */
item *item_get_maxpriority(void) {
    item *it = NULL;
    pthread_mutex_lock(&cache_lock);
    if( pq_get(memcache_pq, (void*)pqdata) == 0 ) {
        it = pqdata->item;
        if(it != NULL){
            it->refcount++;
            do_item_unlink(it);
 ...
 * 查看优先级最高的数据
 */
item *item_look_maxpriority(void) {
    item *it = NULL;
    pthread_mutex_lock(&cache_lock);
    if( pq_look(memcache_pq, (void*)pqdata) == 0) {
        it = pqdata->item;
        it->refcount++;
    }
    pthread_mutex_unlock(&cache_lock);
...
enum store_item_type store_item(item *item, int comm, conn* c) {
    ...
    pthread_mutex_lock(&cache_lock);
    if( pq_is_full(memcache_pq) ) {
        memcache_pq->otimes++;
    }else if( (ret = do_store_item(item, comm, c)  ) == STORED ){
        pqdata->priority    = item->priority;
        pqdata->item        = item;
        pq_insert( memcache_pq, (void*)pqdata);
    }
    pthread_mutex_unlock(&cache_lock);
    return ret;
}
...
void threadlocal_stats_reset(void) {
    ...
    //必须持有cache_lock才能修改队列的状态,因为状态修改的锁就是复用cache_lock
    pthread_mutex_lock(&cache_lock);
    pq_stat_reset(memcache_pq);
    pthread_mutex_unlock(&cache_lock);
}
...
void pqueue_stats(ADD_STAT add_stats, void *c) {
    pthread_mutex_lock(&cache_lock);
    APPEND_STAT("pqueue_size",   "%lu", (unsigned long)memcache_pq->size);
    APPEND_STAT("pqueue_itimes", "%llu", (unsigned long long)memcache_pq->itimes);
    APPEND_STAT("pqueue_gtimes", "%llu", (unsigned long long)memcache_pq->gtimes);
    APPEND_STAT("pqueue_ltimes", "%llu", (unsigned long long)memcache_pq->ltimes);
    APPEND_STAT("pqueue_etimes", "%llu", (unsigned long long)memcache_pq->etimes);
    APPEND_STAT("pqueue_ftimes", "%llu", (unsigned long long)memcache_pq->ftimes);
    APPEND_STAT("pqueue_otimes", "%llu", (unsigned long long)memcache_pq->otimes);
    pthread_mutex_unlock(&cache_lock);
}
