...
#define KEY_TOKEN 1
#define KEY_PRIORITY_TOKEN 2

#define MAX_TOKENS 8
static size_t tokenize_command(char *command, token_t *tokens, const size_t max_tokens);
static int mpq_cmp(void* a, void* b){
    if( ((mpq*)a)->priority == ((mpq*)b)->priority ) return 0;
    return ((mpq*)a)->priority > ((mpq*)b)->priority ? 1 : -1;
}
...
static void settings_init(void) {
    ...
    settings.evict_to_free = 0;       /* push old items out of cache when memory runs out */
...
static void process_bin_sasl_auth(conn *c) {
    ...
    item *it = item_alloc(key, nkey, 0, 0, vlen, 0);
...
static void process_bin_update(conn *c) {
	...
	if(c->cmd == PROTOCOL_BINARY_CMD_ADD) {
		...
		size_t ntokens = tokenize_command(key, tokens, MAX_TOKENS);
		if(ntokens == 3) {
			key = tokens[0].value;
			nkey = tokens[0].length;
			if (! (safe_strtoul(tokens[1].value, (uint32_t *)&priority))) {
    ...
    it = item_alloc(key, nkey, req->message.body.flags,
            realtime(req->message.body.expiration), vlen+2, priority);
...
static void process_bin_append_prepend(conn *c) {
    ...
    it = item_alloc(key, nkey, 0, 0, vlen+2, 0);
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                new_it = do_item_alloc(key, it->nkey, flags, old_it->exptime, it->nbytes + old_it->nbytes - 2 /* CRLF */, it->priority);
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("queue_size_max", "%d", settings.max_q_size);
}
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    size_t nkey;
    unsigned int flags;
    int32_t exptime_int = 0, priority = 0;
    time_t exptime;
    int vlen;
    ...
    if(comm == NREAD_ADD) {
        tokens++;
        if (! (safe_strtoul(tokens[KEY_TOKEN].value, (uint32_t *)&priority))) {
            out_string(c, "CLIENT_ERROR priority must be a numberic string");
            return;
        }
    }
    ...
    it = item_alloc(key, nkey, flags, realtime(exptime), vlen, priority);
...
enum delta_result_type do_add_delta(conn *c, item *it, const bool incr,
    ...
    if (res + 2 > it->nbytes) { /* need to realloc */
        item *new_it;
        new_it = do_item_alloc(ITEM_key(it), it->nkey, atoi(ITEM_suffix(it) + 1),
        it->exptime, res + 2, it->priority);
        if (new_it == 0) {
            return EOM;
        }
...
static void process_delete_command(conn *c, token_t *tokens, const size_t ntokens) {
    out_string(c, "CLIENT_ERROR delete is not support.");
    return;
}

static inline void process_max_priority_get_command(conn *c, token_t *tokens, size_t ntokens, bool fetch) {
    int i = 0;
    item *it;
    //所有key的指针
    assert(c != NULL);
    ...
    if( fetch )
        it = item_get_maxpriority();
    else
        it = item_look_maxpriority();
    //获得到了对应的key的数据
    if (it) {
...
static void process_command(conn *c, char *command) {
    ...
    if (ntokens >= 3 &&
        ...
        if( strcmp( tokens[KEY_TOKEN].value, "maxpriority") == 0)
            process_max_priority_get_command(c, tokens, ntokens, true);
        else if( strcmp( tokens[KEY_TOKEN].value, "maxprioritylook") == 0 )
            process_max_priority_get_command(c, tokens, ntokens, false);
        else
    ...
    } else if (ntokens >= 2 && ntokens <= 4 && (strcmp(tokens[COMMAND_TOKEN].value, "flushallpq") == 0)) {
        time_t exptime = 0;
        set_current_time();
...
#endif
    printf("-Q            Size of priority queue in m, default is 4m\n");
    return;
}
...
int main (int argc, char **argv) {
    ...
    while (-1 != (c = getopt(argc, argv,
        ...
        case 'n':
            ...
            break;
        case 'Q':
            settings.max_q_size = atoi(optarg) * 1024 * 1024;
            if (settings.max_q_size == 0 || settings.max_q_size >= 1024 * 1024 * 1024) {
                fprintf(stderr, "Priority queue size must be greater than 0 and less than 1G\n");
                return 1;
            }
            break;
        case 't':
    ...
    if(memcache_pq == NULL) {
        fprintf(stderr, "failed to malloc mem for priority queue\n");
        exit(EXIT_FAILURE);
    }
